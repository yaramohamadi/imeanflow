...
if [[ $# -lt 2 ]]; then
  cat <<'EOF'
Usage: CONFIG_MODE=caltech_finetune USE_WANDB=False bash scripts/eval_checkpoint_steps_cpu.sh <checkpoint_dir> <num_steps...>

Example:
  CONFIG_MODE=caltech_finetune USE_WANDB=False bash scripts/eval_checkpoint_steps_cpu.sh \
    files/logs/debug/debug_caltech_SiT_train_only_trainonly_20260328_183515_bfndsf/checkpoint_50 2 4

This script evaluates a checkpoint on CPU to avoid GPU restore OOM.
EOF
  exit 1
fi
...
for NUM_STEPS in "${STEPS[@]}"; do
  ...
  EVAL_WORKDIR="$LOG_DIR/eval_cpu_$(basename "$CHECKPOINT_DIR")_${NUM_STEPS}steps"
