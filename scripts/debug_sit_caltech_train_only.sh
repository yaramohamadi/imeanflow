...
if [[ $# -lt 1 ]]; then
  cat <<'EOF'
Usage: CONFIG_MODE=caltech_finetune USE_WANDB=False bash scripts/debug_sit_caltech_train_only.sh <run_label>

Example:
  CONFIG_MODE=caltech_finetune USE_WANDB=False bash scripts/debug_sit_caltech_train_only.sh trainonly

This script:
  1) runs a short SiT Caltech finetuning run without in-training sample/FID generation
...
JOBNAME="debug_caltech_SiT_train_only_${RUN_LABEL}_${NOW}_${SALT}"
