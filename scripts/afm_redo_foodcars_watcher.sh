...
cd "$ADV"
LOG="$ADV/files/logs/afm_redo_foodcars_watcher.log"
mkdir -p "$ADV/files/logs"

POLL_S=60
STABLE_NEEDED=3          # claim promptly as GPUs free
MEM_MB=3000
UTIL_PCT=15
...
STAMP=20260727_redo
PATIENCE=5               # early-stop ON: the other 3 AFM-redo runs all diverged
                         # early; patience-5 stops food/cars once 4-step FID
                         # rises 5 consecutive evals from min (best_fid retained).
...
# ONLY the 2 datasets that never got a GPU in the first redo (food, cars).
QUEUE=(
...
train_done(){  # $1=ds
  ...
  screen -ls | grep -qE "afmrf_gpu[0-9]+_${1}\b" && return 1
  return 0
}
...
while read -r sess; do
  [[ "$sess" =~ afmrf_gpu([0-9]+)_([a-z0-9]+) ]] || continue
  gidx="${BASH_REMATCH[1]}"; gds="${BASH_REMATCH[2]}"
  CLAIMED[$gidx]="$gds"; gpu_try_claim "$gidx" "afmrf_gpu${gidx}_${gds}" >/dev/null 2>&1
  log "ADOPT in-flight $gds on GPU $gidx (screen afmrf_gpu${gidx}_${gds})"
done < <(screen -ls 2>/dev/null | grep -oE "afmrf_gpu[0-9]+_[a-z0-9]+")
...
while (( $(remaining) > 0 )); do
  ...
  for idx in "${!CLAIMED[@]}"; do
    ds="${CLAIMED[$idx]}"
    if screen -ls | grep -qE "afmrf_gpu${idx}_${ds}\b"; then
      ...
      if (( streak >= PATIENCE )); then
        ...
        log "EARLY-STOP $ds: 4-step FID rose $streak consec evals from min. Killing afmrf_gpu${idx}_${ds}."
        screen -S "afmrf_gpu${idx}_${ds}" -X quit 2>/dev/null; sleep 3
      else
        continue
      fi
    fi
  ...
  for idx in $ALLOWED; do
    ...
    for entry in "${QUEUE[@]}"; do
      ...
      (( inflight )) && continue
      gpu_try_claim "$idx" "afmrf_gpu${idx}_${ds}" || { STABLE[$idx]=0; break; }
