...
# on CUB-200-2011. dev5 paths. Override GPUs: CUDA_VISIBLE_DEVICES=2,3 ./train_cub_ddpme.sh
...
JOBNAME="cub_plain_SiT_ddpme_taylor_${NOW}_${SALT}"
WORKDIR="${LOG_DIR}/finetuning/${JOBNAME}"
mkdir -p "${WORKDIR}"

echo "=========================================="
echo "Training DiT->SiT DDPM-e on CUB-200-2011"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" \
  ...
  "${PYTHON}" "${REPO_ROOT}/main_sit.py" \
    ...
    --config.dataset.root="${DATA_ROOT}/cub-200-2011_processed_latents" \
    ...
    --config.logging.wandb_name="CUB_SIT_plain_DiTinit_DDPM-e" \
