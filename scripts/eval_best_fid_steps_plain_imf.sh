...
if [[ $# -lt 1 ]]; then
  cat <<'EOF'
Usage: CONFIG_MODE=plain_imf_finetune bash scripts/eval_best_fid_steps_plain_imf.sh <best_fid_dir_or_run_dir> [num_steps...] [-- extra config overrides...]

Example:
  CONFIG_MODE=plain_imf_finetune bash scripts/eval_best_fid_steps_plain_imf.sh \
    files/logs/finetuning/plain_iMF_finetune_caltech101_baseline_20260418_123456_ab12cd 1 2 250
...
Any args after -- are forwarded to main.py before the eval-only load_from/num_steps overrides.
EOF
  exit 1
fi
