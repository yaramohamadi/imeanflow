#!/usr/bin/env bash
# =============================================================================
# CA-iMF CP-ablation watcher -- STANFORD-CARS (detached: screen caimf_cpabl_cars).
#
# Extends the CP-regularization ablation (previously CUB-200 only) to Cars.
# FT+CAMF: initialize from the Cars TARGET-FINETUNED iMF/XL-2 checkpoint
...
# dataset section for Cars (root, num_classes=10 x3, FID/FDD cache_ref).
#
...
# Cars CAMF run (artbench10_iMF_CAIMF_puresadv_20260726).
#
...
cd "$ADV"
LOG="$ADV/files/logs/caimf_cpabl_cars_watcher.log"
...
MEM_MB=3000
UTIL_PCT=15
ALLOWED="0 1 2 3 4 5"            # Cars pool, avoids ArtBench 6/7
PATIENCE=5               # consecutive 4-step-FID rises -> early stop
STAMP=20260730
CKPT=$MAIN/files/logs/finetuning/plain_iMF_finetune_stanfordcars_h100_20260722_233147_5fimmh/best_fid/checkpoint_12500

# --- Cars dataset overrides (config is CUB by default) ---
ART_ROOT=/opt/dlami/nvme/meanflow/datasets/stanford-cars_processed_latents
ART_NC=196
ART_FID=$MAIN/files/fid_stats/stanford_cars_processed-fid_stats.npz
ART_FDD=$MAIN/files/fdd_stats/stanford-cars-fd_dino-vitb14_stats.npz

log(){ echo "[$(date '+%F %T')] $*" | tee -a "$LOG"; }

MODES=(afm_sum2 none zt zt_zr zt_zhatr full_sum2)
workdir_for(){ echo "$ADV/files/logs/finetuning/stanfordcars_iMF_CAIMF_cpabl_${1}_${STAMP}"; }

train_done(){  # $1=mode
  local wd; wd=$(workdir_for "$1")
  ls -d "$wd/best_fid/checkpoint_"* >/dev/null 2>&1 || return 1
  screen -ls | grep -qE "cpablcars_gpu[0-9]+_${1}\b" && return 1
  return 0
}
...
launch(){  # $1=gpu $2=mode
  ...
  local sess="cpablcars_gpu${gpu}_${mode}"
  log "LAUNCH cp-ablation(Cars) $mode on GPU $gpu (screen $sess) wd=$wd"
  screen -dmS "$sess" bash -c "
    ...
    $PY main_caimf.py \
      ...
      2>&1 | tee -a $ADV/files/logs/caimf_cpabl_cars_${mode}.log
  "
}
...
log "=== CP-ablation Cars watcher: GPUs {$ALLOWED}, ${#MODES[@]} variants, 150k cap, gate $STABLE_NEEDED, patience $PATIENCE, ckpt=$CKPT ==="
...
while read -r sess; do
  [[ "$sess" =~ cpablcars_gpu([0-9]+)_([a-z0-9_]+) ]] || continue
  gidx="${BASH_REMATCH[1]}"; gmode="${BASH_REMATCH[2]}"
  CLAIMED[$gidx]="$gmode"; log "ADOPT in-flight $gmode on GPU $gidx"
done < <(screen -ls 2>/dev/null | grep -oE "cpablcars_gpu[0-9]+_[a-z0-9_]+")
...
while (( $(remaining) > 0 )); do
  ...
  for idx in "${!CLAIMED[@]}"; do
    mode="${CLAIMED[$idx]}"
    if screen -ls | grep -qE "cpablcars_gpu${idx}_${mode}\b"; then
      ...
      if (( streak >= PATIENCE )); then
        ...
        log "EARLY-STOP $mode: 4-step FID rose $streak consecutive evals (min=$mn last=$last, n=$n). Killing cpablcars_gpu${idx}_${mode}."
        screen -S "cpablcars_gpu${idx}_${mode}" -X quit 2>/dev/null; sleep 3
      else
        continue
      fi
    fi
    if ! screen -ls | grep -qE "cpablcars_gpu${idx}_${mode}\b"; then
      if train_done "$mode"; then DONE[$mode]=1; log "COMPLETE $mode (GPU $idx freed)"; else log "WARN $mode screen gone, no best_fid -- check caimf_cpabl_cars_${mode}.log"; DONE[$mode]=1; fi
...
log "=== all 6 Cars CP-ablation variants complete; watcher exiting ==="
