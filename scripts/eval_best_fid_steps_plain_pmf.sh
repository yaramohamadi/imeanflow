...
if [[ $# -lt 1 ]]; then
  cat <<'EOF'
Usage: CONFIG_MODE=plain_pmf_finetune bash scripts/eval_best_fid_steps_plain_pmf.sh <best_fid_dir_or_run_dir> [num_steps...] [-- extra config overrides...]

Example:
  CONFIG_MODE=plain_pmf_finetune bash scripts/eval_best_fid_steps_plain_pmf.sh \
    files/logs/finetuning/plain_pMF_finetune_caltech101_run 1 4
...
Any args after -- are forwarded to main_pmf.py before the eval-only load_from/num_steps overrides.
EOF
  exit 1
fi
...
BEST_FID_DIR="$(cd "$BEST_FID_DIR" && pwd)"
