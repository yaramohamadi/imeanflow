...
# =============================================================================
# Train DiT→SiT Transport-v on Stanford-Cars
# Override GPUs: CUDA_VISIBLE_DEVICES=2,3 ./train_stanford_transportv.sh
...
JOBNAME="stanford_plain_SiT_transportv_taylor_${NOW}_${SALT}"
WORKDIR="${LOG_DIR}/finetuning/${JOBNAME}"
mkdir -p "${WORKDIR}"

echo "=========================================="
echo "Training DiT→SiT Transport-v on Stanford-Cars"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" \
  ...
  "${PYTHON}" "${REPO_ROOT}/main_sit.py" \
    ...
    --config.logging.wandb_name="STANFORD_SIT_plain_DiTinit_Transport-v" \
