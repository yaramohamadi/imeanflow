...
# =============================================================================
# Train DiT→SiT DDPM-v on ArtBench-10
# Override GPUs: CUDA_VISIBLE_DEVICES=2,3 ./train_artbench_ddpmv.sh
...
JOBNAME="artbench_plain_SiT_ddpmv_taylor_${NOW}_${SALT}"
WORKDIR="${LOG_DIR}/finetuning/${JOBNAME}"
mkdir -p "${WORKDIR}"

echo "=========================================="
echo "Training DiT→SiT DDPM-v on ArtBench-10"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" \
  ...
  "${PYTHON}" "${REPO_ROOT}/main_sit.py" \
    ...
    --config.logging.wandb_name="ARTBENCH_SIT_plain_DiTinit_DDPM-v" \
