#!/usr/bin/env bash
# =============================================================================
# CAIMF (pure-adversarial, lambda_imf=0) on NATIVE iMF checkpoints.
# Detached: screen caimf_imf_watcher.
#
# For each of the 5 plain-iMF (imfDiT_XL_2) finetuned students, runs
# finite-interval CA-iMF adversarial post-training OFF its best_fid checkpoint,
# on any stably-free GPU. Uses main_caimf.py DIRECTLY (train_caimf accepts
# model_str=imfDiT_* natively -> no _sit_meft shim, no guard relaxation).
#
# Config: configs/caltech_imf_caimf_posttrain_config.yml
...
# GPU locking: shares the atomic lock with the DiT/JiT CAMF watchers so no two
...
cd "$ADV"
LOG="$ADV/files/logs/caimf_imf_watcher.log"
...
# shared atomic GPU lock (coordinates with caimf_dit/jit watchers)
source "$ADV/scripts/gpu_lock.inc.sh"

# ds : best_fid checkpoint (absolute) -- plain-iMF finetuned imfDiT_XL_2 students
QUEUE=(
...
workdir_for(){ echo "$ADV/files/logs/finetuning/${1}_iMF_CAIMF_puresadv_${STAMP}"; }

train_done(){  # $1=ds
  local wd; wd=$(workdir_for "$1")
  ls -d "$wd/best_fid/checkpoint_"* >/dev/null 2>&1 || return 1
  screen -ls | grep -qE "caimfimf_gpu[0-9]+_${1}\b" && return 1
  return 0
}
...
launch(){  # $1=gpu $2=entry
  ...
  local sess="caimfimf_gpu${gpu}_${ds}"
  log "LAUNCH CAIMF-iMF $ds on GPU $gpu (screen $sess) ckpt=$ckpt wd=$wd"
  screen -dmS "$sess" bash -c "
    ...
    export MPLCONFIGDIR=/tmp/mpl-caimfimf-$ds
    bash $RUNNER $ds '$ckpt' '$wd' \
      2>&1 | tee -a $ADV/files/logs/caimfimf_${ds}.log
  "
}

declare -A STABLE=() CLAIMED=() DONE=()
log "=== CAIMF iMF watcher: ${#QUEUE[@]} datasets, pool {$ALLOWED}, 150k batches, eval@5k NFE4 10k-samples, best-fid-only, final-eval NFE{$FINAL_EVAL_STEPS}, gate $STABLE_NEEDED, GPU-locked ==="
...
while read -r sess; do
  [[ "$sess" =~ caimfimf_gpu([0-9]+)_([a-z0-9]+) ]] || continue
  gidx="${BASH_REMATCH[1]}"; gds="${BASH_REMATCH[2]}"
  CLAIMED[$gidx]="$gds"; gpu_try_claim "$gidx" "caimfimf_gpu${gidx}_${gds}" >/dev/null 2>&1
  log "ADOPT in-flight $gds on GPU $gidx (screen caimfimf_gpu${gidx}_${gds})"
done < <(screen -ls 2>/dev/null | grep -oE "caimfimf_gpu[0-9]+_[a-z0-9]+")
...
while (( $(remaining) > 0 )); do
  ...
  for idx in "${!CLAIMED[@]}"; do
    ...
    if screen -ls | grep -qE "caimfimf_gpu${idx}_${ds}\b"; then
      ...
      if (( streak >= PATIENCE )); then
        ...
        log "EARLY-STOP $ds: 4-step FID rose $streak consecutive evals (min=$mn last=$last, n=$n). Killing caimfimf_gpu${idx}_${ds}."
        screen -S "caimfimf_gpu${idx}_${ds}" -X quit 2>/dev/null; sleep 3
      else
        continue
      fi
    fi
    ...
    if ! screen -ls | grep -qE "caimfimf_gpu${idx}_${ds}\b"; then
      if train_done "$ds"; then DONE[$ds]=1; log "COMPLETE $ds (GPU $idx freed)"; else log "WARN $ds screen gone but no best_fid ckpt -- check caimfimf_${ds}.log"; DONE[$ds]=1; fi
  ...
  for idx in $ALLOWED; do
    ...
    for entry in "${QUEUE[@]}"; do
      ...
      (( inflight )) && continue
      gpu_try_claim "$idx" "caimfimf_gpu${idx}_${ds}" || { STABLE[$idx]=0; break; }
...
log "=== all CAIMF iMF runs complete; watcher exiting ==="
