...
SWEEP_LABEL="${1:-plain_4step_best_imf_rerun}"
shift || true
...
Training jobs submitted/written.

After the training jobs finish, run best-checkpoint final eval separately, e.g.:

  SEARCH_ROOTS="<the new sweep dir printed above>" \
    bash scripts/submit_imf_best_checkpoint_eval_slurm.sh
