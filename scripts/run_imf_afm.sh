#!/usr/bin/env bash
# Run AFM (discrete endpoint-adversarial, ablation=afm_only) post-training on a
# NATIVE target-finetuned iMF checkpoint (plain imfDiT_XL_2 improved-MeanFlow,
# NOT a DMF student, and NOT the ImageNet iMF -- the domain-finetuned one).
#
# Usage:
#   bash scripts/run_imf_afm.sh <dataset> <load_from> <workdir>
#
# train_afm.py accepts model_str=imfDiT_* directly, so this uses main_afm.py
# (no _sit_meft shim). Only generator params are restored from <load_from>;
# the AFM discriminator is initialised from G and both optimizers are fresh.
# Final eval (NFE 1 & 2) runs through the plain-imf eval script. One GPU per run
# (CUDA_VISIBLE_DEVICES from caller).
set -euo pipefail
...
CONFIG_MODE="${CONFIG_MODE:-caltech_imf_afm_posttrain}"
MAIN="$REPO/main_afm.py"
...
if [[ -d "$WORKDIR" ]] && find "$WORKDIR" -mindepth 1 -maxdepth 1 -type d -name 'checkpoint_*' -print -quit | grep -q .; then
  ...
  echo "Use a new workdir (AFM does not resume from this launcher)." >&2
  exit 4
fi
mkdir -p "$WORKDIR"
...
echo "AFM-on-iMF workdir: $WORKDIR"
echo "Dataset: $DATASET  (root=$DATA_ROOT, NC=$NC)"
echo "iMF checkpoint: $LOAD_FROM"
echo "Config: $CONFIG_MODE  GPU: $GPU_LIST"

# class_mapping_root="" -> train_afm._class_mapping falls back to
# {str(i): i} for i in range(num_classes) (num_classes_from_data=True).
COMMON_ARGS=(
  --config="$CONFIG_SPEC"
