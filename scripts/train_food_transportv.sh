...
# =============================================================================
# Train DiT→SiT Transport-v on Food-101
# Override GPUs: CUDA_VISIBLE_DEVICES=2,3 ./train_food_transportv.sh
...
JOBNAME="food_plain_SiT_transportv_taylor_${NOW}_${SALT}"
WORKDIR="${LOG_DIR}/finetuning/${JOBNAME}"
mkdir -p "${WORKDIR}"

echo "=========================================="
echo "Training DiT→SiT Transport-v on Food-101"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" \
  ...
  "${PYTHON}" "${REPO_ROOT}/main_sit.py" \
    ...
    --config.logging.wandb_name="FOOD_SIT_plain_DiTinit_Transport-v" \
