...
WORKDIR="${BASE_WORKDIR}/caltech_SiT_DMF_sampled_guidance_context_decoder_only_eval_7p5_interval_0p3_1_${RUN_LABEL}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES_VALUE}" \
  ...
  "${PYTHON}" main.py \
    ...
    --config.sampling.omega=7.5 \
    --config.sampling.t_min=0.3 \
    ...
    --config.model.training_guidance_interval_strategy=sampled \
    --config.model.training_guidance_start_step=0 \
    --config.model.use_context_guidance_conditioning=True \
    --config.model.use_adaln_guidance_scale_conditioning=False \
    --config.model.decoder_only_guidance_conditioning=True \
