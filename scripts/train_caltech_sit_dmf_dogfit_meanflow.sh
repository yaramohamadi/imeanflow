...
if [[ $# -lt 1 ]]; then
  cat <<'EOF'
Usage: USE_WANDB=True bash scripts/train_caltech_sit_dmf_dogfit_meanflow.sh <run_label> [extra main.py args...]

Example:
  USE_WANDB=True bash scripts/train_caltech_sit_dmf_dogfit_meanflow.sh dogfit_sit

This script:
  1) launches the Caltech SiT-DMF DogFit meanflow config
  2) keeps DogFit's fixed guidance scale, cut-off interval, and late-start defaults
...
JOBNAME="caltech_SiT_DMF_DogFit_meanflow_${RUN_LABEL}_${NOW}_${SALT}"
