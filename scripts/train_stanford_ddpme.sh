...
# on Stanford-Cars. dev5 paths. Override GPUs: CUDA_VISIBLE_DEVICES=2,3 ./train_stanford_ddpme.sh
...
JOBNAME="stanford_plain_SiT_ddpme_taylor_${NOW}_${SALT}"
WORKDIR="${LOG_DIR}/finetuning/${JOBNAME}"
mkdir -p "${WORKDIR}"

echo "=========================================="
echo "Training DiT->SiT DDPM-e on Stanford-Cars"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" \
  ...
  "${PYTHON}" "${REPO_ROOT}/main_sit.py" \
    ...
    --config.dataset.root="${DATA_ROOT}/stanford-cars_processed_latents" \
    --config.dataset.name="${DATASET}" \
    --config.dataset.num_workers=0 \
    --config.dataset.num_classes=${NUM_CLASSES} \
    --config.model.num_classes=${NUM_CLASSES} \
    --config.fid.cache_ref="${REPO_ROOT}/files/fid_stats/stanford_cars_processed-fid_stats.npz" \
    --config.fd_dino.cache_ref="${REPO_ROOT}/files/fdd_stats/stanford-cars-fd_dino-vitb14_stats.npz" \
    --config.logging.wandb_name="STANFORD_SIT_plain_DiTinit_DDPM-e" \
