...
if [[ $# -lt 1 ]]; then
  cat <<'EOF'
Usage: USE_WANDB=True bash scripts/train_caltech_sit_dmf_finetune.sh <run_label> [extra main.py args...]

Example:
  USE_WANDB=True bash scripts/train_caltech_sit_dmf_finetune.sh baseline

This script:
  1) launches the default Caltech SiT-DMF finetuning config
  2) keeps the config-defined single-head and fixed-guidance settings
...
JOBNAME="caltech_SiT_DMF_finetune_${RUN_LABEL}_${NOW}_${SALT}"
